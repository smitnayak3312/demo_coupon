...
if(!isset($_SESSION['id']) || $_SESSION['id'] == "") 
 ...
 $id=trim($_GET['intro_id']);
 $getsliderQuery = "SELECT * FROM introscreen WHERE id = '".$id."' ";
  ...
  if(mysqli_num_rows($getsliderResult) > 0){
    ...
    $text_color_code = $introData['text_color_code'];
    $bg_color_code= $introData['bg_color_code'];
    $name = $introData['name'];
    $intro_descirpition = $introData['intro_descirpition'];
    $introImage = $introData['image'];
    if($introImage != ''){
      $introImage = "image/introscreen_image/".$introImage;
    }
...
{  
 ...
 // $result=$urlspd.$newname;
  ...
  { 
      ...
      $query = "update introscreen set name='".$name."',text_color_code='".$text_color_code."',red_text='".$red_text."',green_text='".$green_text."',blue_text='".$blue_text."',bg_color_code='".$bg_color_code."',red_bg='".$red_bg."',green_bg='".$green_bg."',blue_bg='".$blue_bg."',intro_descirpition='".$intro_descirpition."'";
      if($newname != ''){
      $query .= ",image='".$newname."'";
    }
    ...
    $query .= " WHERE id = '".$id."'";
      if(mysqli_query($conn,$query))
      {
          ...
          echo '<script>alert("Introscreen Edit Successfully");</script>';
                ...
                <div class="card">
                  ...
                  <div class="card-body">
                    ...
                    <div class="form-group col-md-5">
                      ...
                      <input type="text" name="intro_descirpition" value="<?php echo $intro_descirpition;?>" class="form-control">
                    </div>
                    </div>
                    <div class="row">
                     <div class="form-group col-md-5">
                      <label>Text color</label>
                      <input type="color" name="text_color_code" value="<?php echo $text_color_code;?>" class="form-control">
                    </div>
                    <div class="form-group col-md-5">
                      <label>Background color</label>
                      <input type="color" name="bg_color_code" value="<?php echo $bg_color_code;?>" class="form-control">
                    </div>
                    </div>
